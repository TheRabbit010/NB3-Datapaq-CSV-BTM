...
# 5. ฟังก์ชันอ่านไฟล์ CSV แบบรองรับสถานะ *OC* (Open Circuit) และ Auto-Trim
def parse_single_file(uploaded_file):
    ...
    metadata = {
        ...
        "title": "-",
        "operator": "-",
        "product": "Battery Thermal Management",
        ...
        if line_str.startswith("#"):
            ...
            if "=" in line_clean:
                ...
                if key.lower() == "title":
                    ...
                    metadata["product"] = val if (val and val != "-") else "Battery Thermal Management"
                elif key.lower() == "site":
...
if uploaded_file:
    ...
    if df.empty:
        ...
        # 📌 อัปเดตตารางโซนเวลาแบบถูกต้องตรงกับพฤติกรรมกราฟ 100%
        if color_shading_mode == "แสดงสีตามโซน (By Zone)":
            ...
            zones_data = [
                ...
                {"Start Time": "00:27:33", "End Time": "00:28:59", "Zone Name": "Exit", "Color": "#90E0EF"}
            ]
            angle_setting = 0

        # 📌 ตัดข้อมูลกราฟให้แสดงแค่วินาทีที่ 1739 (00:28:59) ครอบคลุมโซนทั้งหมดตั้งแต่ต้นจนจบพอดี
        max_view_sec = 1739
        ...
        with col_h2:
            st.markdown(f"""
                <div class="raw-header-box">
                    ...
                    <div><span class="raw-header-key">#product</span> = <span class="raw-header-val">{metadata.get('product', 'Battery Thermal Management')}</span></div>
        ...
        st.markdown("""
            <div style="background-color: #161b22; border: 1px solid #30363d; border-radius: 6px; padding: 12px 18px; font-size: 13px; color: #CCCCCC; margin-top: 10px;">
                ...
                • <b>Dryer Zone:</b> Max Temperature: <b>200 - 375 °C</b> | Dwell Time Above 300°C: <b>> 2:00 min (>120s)</b> | Dwell Time Above 250°C: <b>> 2:00 min (>120s)</b>
            </div>
        """, unsafe_allow_html=True)
